...
if [[ "$current_session" =~ ^[0-9]+$ ]]; then
    ...
    selected=$(fd --type directory --max-depth 3 --exclude .git --exclude node_modules --exclude .venv --hidden . ~ | sort | fzf --reverse --header="select-working-directory" --preview="eza --tree --git-ignore --level 2 --colour=always --icons=always {}")
